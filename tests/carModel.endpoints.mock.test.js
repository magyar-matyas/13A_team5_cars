...
describe('CarModel végpontok — végpontonként mockolt tesztek', () => {
  ...
  test('POST /models - létrehoz azonosítót a számláló alapján (M005)', async () => {
    CarModel.countDocuments = jest.fn().mockResolvedValue(4);
  ...
  test('POST /models - countDocuments hiba kezelve', async () => {
    CarModel.countDocuments = jest.fn().mockRejectedValue(new Error('count fail'));
  ...
  test('PUT /models/:id - siker, visszaadja a frissített modellt', async () => {
    ...
    CarModel.findByIdAndUpdate = jest.fn().mockResolvedValue(updated);
  ...
  test('PUT /models/:id - adatbázis hiba, 500', async () => {
    CarModel.findByIdAndUpdate = jest.fn().mockRejectedValue(new Error('fail'));
  ...
  test('DELETE /models/:id - siker, üzenetet ad vissza', async () => {
    CarModel.findByIdAndDelete = jest.fn().mockResolvedValue({ _id: 'MD' });
  ...
  test('DELETE /models/:id - nem található, 404', async () => {
    CarModel.findByIdAndDelete = jest.fn().mockResolvedValue(null);
  ...
  test('DELETE /models/:id - adatbázis hiba, 500', async () => {
    CarModel.findByIdAndDelete = jest.fn().mockRejectedValue(new Error('fail'));
  ...
  test('DELETE /models/:id - törlés után GET /models üreset ad', async () => {
    CarModel.findByIdAndDelete = jest.fn().mockResolvedValue({ _id: 'MD2' });
    CarModel.find = jest.fn().mockResolvedValue([]);
